...
async function fetchHealth() {
  try {
    const res = await fetch(`${apiUrl}/api/v1/status-page/health`)
    if (res.ok) {
...
async function fetchUptime() {
  try {
    const res = await fetch(`${apiUrl}/api/v1/status-page/uptime?days=90`)
    if (res.ok) {
      const text = await res.text()
...
async function fetchPosts(showLoading = true) {
  ...
  try {
    ...
    const res = await fetch(`${apiUrl}/api/v1/status-page/posts?${params}`)
    if (res.ok) {
      const text = await res.text()
            ...
            <div class="mx-auto max-w-3xl">
              ...
              <div v-else-if="!posts.length" class="py-16 text-center">
                ...
                <p class="text-lg font-medium text-gray-900 dark:text-white">
                  {{ searchQuery ? t('statusPage.timeline.noResults') : t('statusPage.timeline.empty') }}
                </p>
              </div>

...
CREATE INDEX IF NOT EXISTS `idx_accounts_scheduled_deletion` ON `accounts` (`scheduled_deletion_at`);
CREATE INDEX IF NOT EXISTS `idx_nodes_last_heartbeat` ON `nodes` (`last_heartbeat`);
CREATE INDEX IF NOT EXISTS `idx_subscriptions_past_due_since` ON `subscriptions` (`past_due_since`);

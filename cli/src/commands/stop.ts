...
export const stopCommand = new Command('stop')
  ...
  .action(async (service: string) => {
    try {
      ...
      const spinner = ora(`Stopping ${chalk.cyan(service)}...`).start()

      const serviceId = await resolveServiceId(service)
      await apiRequest('POST', `/api/v1/services/${serviceId}/stop`)

      spinner.succeed(chalk.green(`Service ${chalk.cyan(service)} stopped.`))
    } catch (err) {
